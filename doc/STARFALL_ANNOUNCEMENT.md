Please be aware that this incident is **ONGOING**, and the report is by no means complete.  
Details may change or become available in the future.

The following report details events pertaining to a denial-of-service campaign directed at Starfall.  
It contains an important security advisory. Please share with others you think may be impacted, or are at-risk.

## Table of Contents

1. [The Timeline](#the-timeline)
2. [The Evidence](#the-evidence)
    1. [Firewall Logs](#firewall-logs)
    2. [A Lesson in Operations Security](#a-lesson-in-operations-security)
3. [The Exploit](#the-exploit)
    1. [Mitigations](#mitigations)
4. [Conclusion](#conclusion)
5. [TLDR](#tldr)

## The Timeline

For clarity's sake, all timestamps are in EST unless written otherwise.

1. July 20th, 4:30 AM - Accounts became required to view some portions of the website  
   We made the unfortunate decision to lock down the website after receiving multiple reports
   of users being harassed for having an account on Starfall. We still regret doing this, but we
   are working on an opt-in visibility feature, so the site can remain open.

2. July 20th, 8:30 PM - Reports of gameserver connection issues  
   Players who were online at the time were complaining of connection issues in-game.  
   This causes us to investigate what the issue could be.

3. July 21st, 12:48 AM - The account requirement was temporarily disabled for diagnosing  
   While the initial commit should not have caused the reported issues, the requirement was temporarily
   removed during the investigation to eliminate all possible causes.

4. July 21st, 1:00 AM - Foul play identified  
   A test was conducted where all network traffic, excluding one invidividual, was blocked
   from the gameserver.

    The test was a success, and we became aware the reported issue was caused by an individual
    performing a denial-of-service attack on the gameservers.

5. July 21st, 2:00 AM - Attacker's identity verified  
   After some digging into how the exploit works, we decided to figure out who this attacker was.
   Search engines revealed that the offending IP address belonged to XlXi, owner and operator of the
   unfinished revival VirtuBrick.

6. July 21st, 3:00 AM - Account requirement was re-enabled  
   Following the confirmation that the requirement was not the cause of the issue, it was re-enabled.

7. July 21st, 4:00 AM - Packet captures finalized, initial firewall rules created  
   At this point, we now have ~200 MB of raw captured traffic from the gameserver VM, all confirming
   that XlXi is performing this attack.

## The Evidence

To most of you, this is not a surprise. But, others might be left wondering who XlXi is, and why he would do this.
We aren't going to pretend like we know why this guy is attacking Starfall, since every attempt at conversation with him
ends up going nowhere, or results in "Critical thinking skills" coming into question.
Just classic Kyle shit. (Gee, this guy hasn't changed at all, huh?)

So, heres the evidence that XlXi is in fact, doing this.  
(Please note, packet captures will not be available until our patches are released, so that copycats cannot make this issue MUCH larger than it is already.)

### Firewall Logs

```
103 4 tap103i0-IN 21/Jul/2026:07:12:02 -0400 DROP: IN=fwbr103i0 OUT=fwbr103i0 PHYSIN=fwln103i0 PHYSOUT=tap103i0 MAC=bc:24:11:15:67:67:bc:f8:7e:8e:2a:ee:08:00 SRC=104.61.230.122 DST=108.18.94.49 LEN=53 TOS=0x00 PREC=0x00 TTL=54 ID=11418 DF PROTO=UDP SPT=49795 DPT=23700 LEN=33
103 4 tap103i0-IN 21/Jul/2026:07:12:02 -0400 DROP: IN=fwbr103i0 OUT=fwbr103i0 PHYSIN=fwln103i0 PHYSOUT=tap103i0 MAC=bc:24:11:15:67:67:bc:f8:7e:8e:2a:ee:08:00 SRC=104.61.230.122 DST=108.18.94.49 LEN=53 TOS=0x00 PREC=0x00 TTL=54 ID=58561 DF PROTO=UDP SPT=38870 DPT=23701 LEN=33
103 4 tap103i0-IN 21/Jul/2026:07:12:02 -0400 DROP: IN=fwbr103i0 OUT=fwbr103i0 PHYSIN=fwln103i0 PHYSOUT=tap103i0 MAC=bc:24:11:15:67:67:bc:f8:7e:8e:2a:ee:08:00 SRC=104.61.230.122 DST=108.18.94.49 LEN=53 TOS=0x00 PREC=0x00 TTL=54 ID=5911 DF PROTO=UDP SPT=36436 DPT=23702 LEN=33
103 4 tap103i0-IN 21/Jul/2026:07:12:02 -0400 DROP: IN=fwbr103i0 OUT=fwbr103i0 PHYSIN=fwln103i0 PHYSOUT=tap103i0 MAC=bc:24:11:15:67:67:bc:f8:7e:8e:2a:ee:08:00 SRC=104.61.230.122 DST=108.18.94.49 LEN=53 TOS=0x00 PREC=0x00 TTL=54 ID=31920 DF PROTO=UDP SPT=48928 DPT=23703 LEN=33
103 4 tap103i0-IN 21/Jul/2026:07:12:02 -0400 DROP: IN=fwbr103i0 OUT=fwbr103i0 PHYSIN=fwln103i0 PHYSOUT=tap103i0 MAC=bc:24:11:15:67:67:bc:f8:7e:8e:2a:ee:08:00 SRC=104.61.230.122 DST=108.18.94.49 LEN=53 TOS=0x00 PREC=0x00 TTL=54 ID=5295 DF PROTO=UDP SPT=46811 DPT=23705 LEN=33
```

To the average person, this might be hard to read. But I'll break it down for you.

The firewall is configured to `DROP` all traffic originating from `104.61.230.122`, and to create a log entry.  
Each line represents every time a packet was discarded. But, the important parts are as follows:

- `SRC=`: The source IP. Where the traffic is coming from.
- `DST=`: The destination IP. Where the traffic is going.
- `PROTO=`: The protocol of the traffic. What a protocol is, isn't important right now, but let it be known that Starfall **only** uses `UDP`
- `SPT=`: The source port. Again, where the traffic is coming from. Usually random for every connection.
- `DPT=`: The destination port. This is used to determine what service on the IP the traffic should be routed to.  
  Starfall uses port ranges `23700` to `23799`. If `DPT` is within that range, the traffic is for Starfall.

Great. It looks like someone with the IP address `104.61.230.122` is repeatedly trying to contact Starfall gameservers. What does that prove?  
By itself, it proves nothing. But, when you associate the IP address with the person behind it, the nature becomes more apparent.

### A Lesson in Operations Security

XlXi has a history of poor decisions like this. Previously, he organized an IP logging campaign on Tadah, a now dead ROBLOX revival,
on the same IP address directly associated with him at the time. Now, he's done the same thing.

Dropping this IP address into [Shodan](https://www.shodan.io/), reveals that it exposes a web server.  
The TLS certificate this web server defaults is bound to the domain `virtubrick.local`. An unfinished revival that XlXi owns and operates.  
This effectively proves that the malicious traffic we have identified, is coming directly from the network XlXi operates.

Not enough for you? Add the following to your `hosts` file, and navigate to [www.virtubrick.local](https://www.virtubrick.local/) yourself!

```
104.61.230.122 www.virtubrick.local
```

- [Shodan Results](https://www.shodan.io/host/104.61.230.122)
    - [Archive 1](https://web.archive.org/web/20260721152713/https://www.shodan.io/host/104.61.230.122)
    - [Archive 2](http://archive.today/2026.07.21-153011/https://www.shodan.io/host/104.61.230.122)

## The Exploit

We have spent some time reverse-engineering the exploit XlXi is using to cause these service disruptions. It's plainly simple, but powerful.  
Currently, it has been confirmed that the exploit XlXi is using affects **AT LEAST** ROBLOX client versions `0.78.0.701` to `0.360.1.252096`.  
This is why we are currently not releasing our packet captures, proof-of-concept exploit, or extended details.

### Mitigations

Literally just block his IP address. This works for now, but I'm sure he will start using a VPN or proxy to get around it.

Starfall has a lower-level mitigation already deployed, and some revivals have already gotten this.  
Keep an eye out for our offically endorsed patch on Hexagon's blog. They have been gracious enough to allow us to use their platform.

Additionally, he has since stopped these attacks after being called out privately. (If you weren't doing anything other than port scanning, why stop? ;) )

## Conclusion

That pretty much wraps it up for the exploit, at least. Theres a huge amount of disinformation about Starfall coming from XlXi that we'll just have
to cover some other time. While we are working on that, I personally hope this report is enough justification to finally cut this guy off, if you haven't
already. He sits on a cache of old ROBLOX vulnerabilities and decides to use then whenever he pleases, acting as some sort of old ROBLOX vigilante.
Don't trust people who would rather commit actual felonies than talk things out, no matter if they're on your side or not.

## TLDR

- XlXi has been conducting a denial-of-service attack that uses a vulnerability in the client itself.
- The exploit works on clients ranging from 2012 all the way up to 2018.
- Revival operators are urged to apply firewall rules blocking his traffic, until the team at Starfall releases an open-source fix.
- Please be weary of his conduct and understand that he is not a trustworthy individual.
